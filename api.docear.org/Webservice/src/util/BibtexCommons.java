...
public class BibtexCommons {
    ...
    public String format(String field) {
		...
		for (i = 0; i < field.length(); i++) {
			...
			if (escaped && (c == '\\')) {
				...
				|| (SPECIAL_COMMAND_CHARS.indexOf(String.valueOf(c)) >= 0)) {
				escaped = false;
                ...
                if (!incommand)
                    ...
                    testCharCom: if ((currentCommand.length() == 1)
						&& (SPECIAL_COMMAND_CHARS.indexOf(currentCommand.toString()) >= 0)) {
	...
	public static String prepareForSave(String bib) {
		if(bib == null) {
			return null;
		}
		int start = bib.indexOf("{");
		int end = bib.indexOf(",", start);
		if(start > -1 && end > -1) {
			return bib.replaceFirst("\\"+bib.subSequence(start, end).toString(), "{").trim()+"\n";
		}
		return bib;
	}
	
	public static String prepareBibEntry(String bib, Integer bibID) {
		if(bib == null) {
			return null;
		}
		int start = bib.indexOf("{");
		int end = bib.indexOf(",", start);
		if(start > -1 && end > -1) {
			return bib.replaceFirst("\\"+bib.subSequence(start, end).toString(), "{,\n  dcr_bibtex_id={"+Integer.toString(bibID)+"}");
		}
		return bib;
	}
}
