...
def _create_tmp_rec_clicked(gap, auto, approach):
    ...
    if approach == 0:
        ...
        query += " AND data_element_type = 2 AND approach = 1"
    query += """ group by user_id, delivered, fulltext_url_id"""
#    print(query)
    db.query(query)
    
    #CLEAN BASE DATA FROM SESSIONS WITH LESS THAN 10 DOCUMENTS
#    query = """DELETE A FROM tmp_rec_base A JOIN (select iteration, user_id, delivered, user_model_id, count(*) from tmp_rec_base
#            GROUP BY iteration, user_id, delivered, user_model_id
#            HAVING COUNT(*) <> 10) B
#            ON (A.iteration=B.iteration AND A.user_id=B.user_id AND A.delivered= B.delivered AND A.user_model_id = B.user_model_id)"""
    ...
    for i in range(2, 51):
        ...
        if approach == 0:
            ...
            query += " AND data_element_type = 2 AND approach = 1"               
        query += """ group by R.user_id, R.delivered, R.fulltext_url_id"""
#        print(query)        
        db.query(query)
    ##########################################################
...
def _create_results(gap, auto, approach):    
    ...
    for i in range (1,51):
        ...
        query = """UPDATE """+tablename+""" T JOIN
                ...
                (SELECT user_id, delivered, fulltext_url_id FROM tmp_rec_clicked WHERE iteration="""+str(i)+""" AND clicked IS NOT NULL) B
                ON (A.user_id = B.user_id AND A.delivered = B.delivered AND A.fulltext_url_id <> B.fulltext_url_id)
                WHERE A.iteration = 1
...
def _standard(approach):
     ...
     _create_tmp_rec_clicked(0, -1, approach)
     _create_results(0, -1, approach)
